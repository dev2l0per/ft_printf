...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/10/31 19:52:01 by juyang            #+#    #+#             */
/*   Updated: 2020/10/31 19:52:02 by juyang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			print_hex_upper(t_info *info, va_list ap)
{
	...
	if (info->left_align)
		res += (info->zero) ?
		print_hex_upper_align_zero(info, lld, len, str)
		: print_hex_upper_align(info, lld, len, str);
	else
		res += (info->zero) ?
		print_hex_upper_zero(info, lld, len, str)
		: print_hex_upper_noflag(info, lld, len, str);
	return (res);
}
